...
export default function WorkflowNode({ node, onEdit, onDelete, isActive }) {
  ...
  const colorClass =
    nodeColors[node.type] || "bg-gray-500/20 border-gray-500 text-gray-400";

  return (
    <Card
      className={`p-4 border-2 transition-all ${colorClass} ${isActive ? "shadow-lg scale-105" : ""}`}
    >
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {node.type === "condition" && (
            <Badge className="mt-2 text-xs">If {node.config?.condition}</Badge>
          )}
        </div>
