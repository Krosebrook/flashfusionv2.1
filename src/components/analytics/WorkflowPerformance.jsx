...
export default function WorkflowPerformance({ workflows, tasks }) {
  ...
  const workflowMetrics = workflows.map((workflow) => {
    const workflowTasks = tasks.filter(
      (t) =>
        t.project_id === workflow.id ||
        t.project_id === `workflow_${workflow.id}`
    );
    const totalExecutions = workflow.execution_count || 0;
    const successfulExecutions = workflowTasks.filter(
      (t) => t.status === "completed"
    ).length;
    const failedExecutions = workflowTasks.filter(
      (t) => t.status === "failed"
    ).length;
    const avgExecutionTime =
      workflowTasks.length > 0
        ? workflowTasks.reduce((sum, t) => sum + (t.execution_time || 0), 0) /
          workflowTasks.length
        : 0;
    const totalCredits = workflowTasks.reduce(
      (sum, t) => sum + (t.credits_used || 0),
      0
    );
    const successRate =
      totalExecutions > 0
  ...
  const totalWorkflowExecutions = workflowMetrics.reduce(
    (sum, w) => sum + w.totalExecutions,
    0
  );
  const avgSuccessRate =
    workflowMetrics.length > 0
      ? workflowMetrics.reduce((sum, w) => sum + w.successRate, 0) /
        workflowMetrics.length
      : 0;
  const totalWorkflowCredits = workflowMetrics.reduce(
    (sum, w) => sum + w.totalCredits,
    0
  );
  ...
  const bottlenecks = workflowMetrics
    ...
    .map((w) => ({
      ...
      issues: [
        ...
        w.avgExecutionTime > 30 &&
          `Slow execution (${w.avgExecutionTime}s avg)`,
              ...
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-400">
                    {workflow.nodes} nodes • {workflow.totalExecutions}{" "}
                    executions
                  </p>
                </div>
                <Badge
