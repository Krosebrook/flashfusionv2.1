...
export default function ContentPerformance({ contents }) {
  const contentStats = contents
    .map((content) => ({
      ...
      views:
        content.performance_data?.views || Math.floor(Math.random() * 1000),
      engagement:
        content.performance_data?.engagement || Math.floor(Math.random() * 500),
      shares:
        content.performance_data?.shares || Math.floor(Math.random() * 100),
      type: content.type,
    }))
    .slice(0, 10);

  const totalViews = contentStats.reduce((sum, item) => sum + item.views, 0);
  const totalEngagement = contentStats.reduce(
