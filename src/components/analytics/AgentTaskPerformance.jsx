...
export default function AgentTaskPerformance({ tasks }) {
  ...
  const agentPerformance = Object.values(agentStats).map((agent) => ({
    ...agent,
    successRate:
      agent.total > 0 ? ((agent.completed / agent.total) * 100).toFixed(1) : 0,
    avgTime:
      agent.completed > 0 ? (agent.totalTime / agent.completed).toFixed(2) : 0,
    avgCredits:
      agent.completed > 0
        ? Math.round(agent.totalCredits / agent.completed)
        : 0,
  }));
  ...
  const unreliableAgents = agentPerformance.filter(
    (a) => parseFloat(a.successRate) < 80 && a.total > 3
  );
  ...
  const avgExecutionTime =
    tasks.length > 0
      ? (
          tasks.reduce((sum, t) => sum + (t.execution_time || 0), 0) /
          tasks.filter((t) => t.execution_time).length
        ).toFixed(2)
      : 0;
  const totalCreditsConsumed = tasks.reduce(
            ...
            <div key={agent.agent} className="bg-gray-900 p-4 rounded-lg">
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <Badge
                    className={
                      parseFloat(agent.successRate) >= 90
                        ? "bg-green-500/20 text-green-400"
                        : parseFloat(agent.successRate) >= 70
