...
export default function AdPerformanceMetrics() {
  ...
  const calculateStats = () => {
    const totalImpressions = filteredAds.reduce(
      (sum, a) => sum + (a.ad_metrics?.impressions || 0),
      0
    );
    const totalClicks = filteredAds.reduce(
      (sum, a) => sum + (a.ad_metrics?.clicks || 0),
      0
    );
    const totalConversions = filteredAds.reduce(
      (sum, a) => sum + (a.ad_metrics?.conversions || 0),
      0
    );
    const totalSpend = filteredAds.reduce(
      (sum, a) => sum + (a.ad_metrics?.spend || 0),
      0
    );
    const totalRevenue = filteredAds.reduce(
      (sum, a) => sum + (a.ad_metrics?.revenue || 0),
      0
    );
  ...
  if (isLoading)
    return <div className="text-gray-400">Loading ad metrics...</div>;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <Card className="bg-gray-800 border-gray-700 p-4">
          ...
          <p
            className={`text-2xl font-bold ${stats.profit >= 0 ? "text-green-400" : "text-red-400"}`}
          >
      ...
      {filteredAds.length > 0 && (
        <Card className="bg-gray-800 border-gray-700 p-6">
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {filteredAds
              .sort(
                (a, b) => (b.ad_metrics?.roas || 0) - (a.ad_metrics?.roas || 0)
              )
              .map((ad) => (
                <div
                  key={ad.id}
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-7 gap-3 text-xs">
                    <div>
                      ...
                      <p className="font-semibold">
                        {(ad.ad_metrics?.impressions || 0).toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-400">Clicks</span>
                      <p className="font-semibold">
                        {(ad.ad_metrics?.clicks || 0).toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-400">CTR</span>
                      <p className="font-semibold">
                        {(ad.ad_metrics?.ctr || 0).toFixed(2)}%
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-400">Spend</span>
                      <p className="font-semibold">
                        ${(ad.ad_metrics?.spend || 0).toFixed(2)}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-400">Conversions</span>
                      <p className="font-semibold text-purple-400">
                        {(ad.ad_metrics?.conversions || 0).toLocaleString()}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-400">CPA</span>
                      <p className="font-semibold">
                        ${(ad.ad_metrics?.cpa || 0).toFixed(2)}
                      </p>
                    </div>
                    <div>
                      <span className="text-gray-400">ROAS</span>
                      <p className="font-semibold text-green-400">
                        {(ad.ad_metrics?.roas || 0).toFixed(2)}x
                      </p>
                    </div>
                  </div>
                </div>
