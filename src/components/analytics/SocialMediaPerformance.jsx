...
export default function SocialMediaPerformance() {
  ...
  const calculateStats = () => {
    return {
      totalImpressions: filteredData.reduce(
        (sum, p) => sum + (p.social_metrics?.impressions || 0),
        0
      ),
      totalEngagement: filteredData.reduce(
        (sum, p) =>
          sum +
          (p.social_metrics?.likes || 0) +
          (p.social_metrics?.comments || 0) +
          (p.social_metrics?.shares || 0),
        0
      ),
      avgEngagementRate:
        filteredData.length > 0
          ? (
              filteredData.reduce(
                (sum, p) => sum + (p.social_metrics?.engagement_rate || 0),
                0
              ) / filteredData.length
            ).toFixed(1)
          : 0,
      totalReach: filteredData.reduce(
        (sum, p) => sum + (p.social_metrics?.reach || 0),
        0
      ),
    };
  };
  ...
  if (isLoading)
    return <div className="text-gray-400">Loading social metrics...</div>;
        ...
        <Card className="bg-gray-800 border-gray-700 p-6">
          ...
          <div className="space-y-3 max-h-96 overflow-y-auto">
            {filteredData
              .sort(
                (a, b) =>
                  (b.social_metrics?.engagement_rate || 0) -
                  (a.social_metrics?.engagement_rate || 0)
              )
              .slice(0, 10)
              .map((post) => (
                <div
                ...
                >
                  <div className="flex justify-between items-start mb-2">
                    <h5 className="font-medium truncate flex-1">
                      {post.content_title}
                    </h5>
                  ...
                  <div className="grid grid-cols-4 gap-2 text-xs">
                    <div>
                      ...
                      <span className="font-semibold">
                        {(
                          post.social_metrics?.impressions || 0
                        ).toLocaleString()}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="font-semibold">
                        {(post.social_metrics?.reach || 0).toLocaleString()}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="font-semibold">
                        {(
                          (post.social_metrics?.likes || 0) +
                          (post.social_metrics?.comments || 0) +
                          (post.social_metrics?.shares || 0)
                        ).toLocaleString()}
                      </span>
                    </div>
                    <div>
                      ...
                      <span className="font-semibold text-green-400">
                        {(post.social_metrics?.engagement_rate || 0).toFixed(1)}
                        %
                      </span>
                    </div>
                  </div>
