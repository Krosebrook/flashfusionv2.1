...
export default function EmailCampaignAnalytics() {
  ...
  const calculateStats = () => {
    const totalSent = campaigns.reduce(
      (sum, c) => sum + (c.email_metrics?.sent || 0),
      0
    );
    const totalOpened = campaigns.reduce(
      (sum, c) => sum + (c.email_metrics?.opened || 0),
      0
    );
    const totalClicked = campaigns.reduce(
      (sum, c) => sum + (c.email_metrics?.clicks || 0),
      0
    );
    const totalConverted = campaigns.reduce(
      (sum, c) => sum + (c.email_metrics?.conversions || 0),
      0
    );
    ...
    return {
      ...
      avgOpenRate:
        campaigns.length > 0
          ? (
              campaigns.reduce(
                (sum, c) => sum + (c.email_metrics?.open_rate || 0),
                0
              ) / campaigns.length
            ).toFixed(1)
          : 0,
      avgClickRate:
        campaigns.length > 0
          ? (
              campaigns.reduce(
                (sum, c) => sum + (c.email_metrics?.click_rate || 0),
                0
              ) / campaigns.length
            ).toFixed(1)
          : 0,
      avgConversionRate:
        campaigns.length > 0
          ? (
              campaigns.reduce(
                (sum, c) => sum + (c.email_metrics?.conversion_rate || 0),
                0
              ) / campaigns.length
            ).toFixed(1)
          : 0,
  ...
  if (isLoading)
    return <div className="text-gray-400">Loading email analytics...</div>;
            ...
            {campaigns.map((campaign) => (
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <Badge
                    variant={
                      campaign.status === "published" ? "default" : "outline"
                    }
                  >
                    {campaign.status}
                  </Badge>
                ...
                <div className="grid grid-cols-2 md:grid-cols-6 gap-3 text-xs">
                  <div>
                    ...
                    <p className="font-semibold">
                      {(campaign.email_metrics?.sent || 0).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <span className="text-gray-400">Opened</span>
                    <p className="font-semibold text-green-400">
                      {(campaign.email_metrics?.opened || 0).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <span className="text-gray-400">Clicked</span>
                    <p className="font-semibold text-blue-400">
                      {(campaign.email_metrics?.clicks || 0).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <span className="text-gray-400">Open Rate</span>
                    <p className="font-semibold">
                      {(campaign.email_metrics?.open_rate || 0).toFixed(1)}%
                    </p>
                  </div>
                  <div>
                    <span className="text-gray-400">Click Rate</span>
                    <p className="font-semibold">
                      {(campaign.email_metrics?.click_rate || 0).toFixed(1)}%
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="font-semibold text-purple-400">
                      {(
                        campaign.email_metrics?.conversions || 0
                      ).toLocaleString()}
                    </p>
                  </div>
                </div>
