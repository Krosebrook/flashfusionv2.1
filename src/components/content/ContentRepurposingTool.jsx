...
Provide a natural, culturally appropriate translation.`;
      ...
      const { translated_title, translated_content } = await base44.integrations.Core.InvokeLLM({
        prompt,
        response_json_schema: {
          type: "object",
          properties: {
  ...
  const handleRepurpose = async () => {
    ...
    try {
      const formatInstructions = {
        social_post: "Convert to a social media post (150-200 words) with hooks and hashtags",
        email: "Transform into an email newsletter format with subject line and clear sections",
        blog: "Expand into a full blog post (500+ words) with introduction, body, and conclusion",
        video_script: "Adapt into a video script with scene directions and voiceover",
        infographic: "Break down into key points suitable for an infographic (5-7 bullet points)"
      };
...
Maintain the core message but optimize for the new format.`;
      ...
      const { repurposed_content, format_specific_data } = await base44.integrations.Core.InvokeLLM({
        prompt,
        response_json_schema: {
          type: "object",
          properties: {
            ...
            <div>
              ...
              <Select value={targetFormat} onValueChange={setTargetFormat}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="infographic">Infographic Points</SelectItem>
                </SelectContent>
              </Select>
            </div>
            <Button
        ...
        <Card className="bg-gray-800 border-green-900/50 border-2 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-xl font-semibold text-green-400">
              ...
              {result.type === 'translation' && `Translated to ${result.language}`}
              {result.type === 'repurposed' && `Repurposed as ${result.format.replace(/_/g, ' ')}`}
            </h3>
          </div>
          ...
          <div className="flex gap-3">
            <Button onClick={handleSaveResult} className="flex-1 bg-green-600 hover:bg-green-700">
              Save as New Content
            </Button>
