...
const InsightCard = ({ title, value, subtitle, icon: Icon, color, alert }) => (
  <Card
    className={`bg-gray-800 border-gray-700 p-4 ${alert ? "border-yellow-500" : ""}`}
  >
...
export default function WSJFAnalytics({ items, history }) {
  ...
  const driftData = useMemo(() => {
    ...
    const recentHistory = history.filter(
      (h) => new Date(h.created_date) >= cutoffDate
    );
    ...
    items.forEach((item) => {
      ...
      const scoreChanges = itemHistory.filter(
        (h) => h.action === "update" && h.old_wsjf_score !== h.new_wsjf_score
      );
        ...
        volatility:
          scoreChanges.length > 0
            ? scoreChanges.reduce(
                (sum, h) =>
                  sum +
                  Math.abs((h.new_wsjf_score || 0) - (h.old_wsjf_score || 0)),
                0
              ) / scoreChanges.length
            : 0,
        scoreTrend:
          scoreChanges.length > 0
            ? (scoreChanges[0]?.new_wsjf_score || 0) -
              (scoreChanges[scoreChanges.length - 1]?.old_wsjf_score || 0)
            : 0,
        uniqueEditors: [...new Set(itemHistory.map((h) => h.actor_email))]
          .length,
    ...
    return {
      ...
      userActivity: Object.values(userActivity).map((u) => ({
        ...u,
        itemsTouched: u.itemsTouched.size,
      })),
      dailyChanges: Object.values(dailyChanges).reverse(),
      totalChanges: recentHistory.length,
      totalScoreChanges: recentHistory.filter(
        (h) => h.action === "update" && h.old_wsjf_score !== h.new_wsjf_score
      ).length,
    };
  ...
  const gamingInsights = useMemo(() => {
    ...
    const highVolatilityItems = driftData.itemDrift.filter(
      (d) => d.scoreChanges >= 3
    );
    ...
    const hyperactiveUsers = driftData.userActivity.filter(
      (u) => u.scoreChanges >= 5
    );
    ...
    const multiEditorItems = driftData.itemDrift.filter(
      (d) => d.uniqueEditors >= 3
    );
    ...
    const bumpedItems = driftData.itemDrift.filter(
      (d) => d.scoreTrend > 3 && d.scoreChanges >= 2
    );
  ...
  const stats = {
    ...
    avgVolatility:
      driftData.itemDrift.length > 0
        ? (
            driftData.itemDrift.reduce((s, d) => s + d.volatility, 0) /
            driftData.itemDrift.length
          ).toFixed(2)
        : 0,
      ...
      <Tabs defaultValue="timeline" className="w-full">
        ...
        <TabsContent value="items" className="space-y-4">
          <Card className="bg-gray-800 border-gray-700 p-4">
            ...
            {driftData.itemDrift.filter((d) => d.scoreChanges > 0).length >
            0 ? (
              <ResponsiveContainer width="100%" height={300}>
                <BarChart
                  data={driftData.itemDrift
              ...
              <p className="text-center text-gray-500 py-8">
                No score changes in selected period
              </p>
            )}
          </Card>
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400">
                        {d.scoreChanges} score changes by {d.uniqueEditors}{" "}
                        editor(s)
                      </p>
                    </div>
                    <div className="text-right">
                      <div
                        className={`flex items-center gap-1 ${d.scoreTrend > 0 ? "text-green-400" : d.scoreTrend < 0 ? "text-red-400" : "text-gray-400"}`}
                      >
                        {d.scoreTrend > 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {driftData.userActivity
              ...
              .map((u, i) => (
                <Card
                  key={i}
                  className={`bg-gray-800 border-gray-700 p-3 ${u.scoreChanges >= 5 ? "border-yellow-500" : ""}`}
                >
