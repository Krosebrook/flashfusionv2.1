...
Deno.serve(async (req) => {
  ...
  const runId = await createReconcileRun(base44, 'custom_api');
  try {
    const config = await getIntegrationConfig(base44, 'custom_api');
    if (!config?.enabled) {
    ...
    const stuck = await base44.asServiceRole.entities.IntegrationOutbox.filter({
      integration_id: 'custom_api', status: 'queued'
    });
    let fixed = 0;
