...
Deno.serve(async (req) => {
  ...
  const runId = await createReconcileRun(base44, 'openai_tts');
  try {
    const config = await getIntegrationConfig(base44, 'openai_tts');
    if (!config?.enabled) {
    ...
    const stuck = await base44.asServiceRole.entities.IntegrationOutbox.filter({
      integration_id: 'openai_tts', status: 'queued'
    });
    let fixed = 0;
