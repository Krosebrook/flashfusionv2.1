...
Deno.serve(async (req) => {
  ...
  const runId = await createReconcileRun(base44, 'elevenlabs');
  try {
    const config = await getIntegrationConfig(base44, 'elevenlabs');
    if (!config?.enabled) {
    ...
    const stuck = await base44.asServiceRole.entities.IntegrationOutbox.filter({
      integration_id: 'elevenlabs', status: 'queued'
    });
    let fixed = 0;
